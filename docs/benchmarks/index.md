...
> **Quick Picks (MacBook Pro M1 Max 64 GB):** See the [homepage TL;DR](../index.md) for the full quick-picks table. In summary: Best all-around → **gemma4:e4b-nvfp4** (60.4 tok/s, 8/10 quality). Best coding → **qwen3-coder:30b** (55.7 tok/s, 10/10 quality). Best MoE → **qwen3.6:35b-a3b-nvfp4** (54.3 tok/s, 9/10 quality). Best small → **gemma4:e4b-nvfp4** (60.4 tok/s, 8/10 quality). Best reasoning → **deepseek-r1:14b** (10/10 quality, but memory-hungry).
...
- **Qwen 3 Coder 30B is the top coding model** at 55.7 tok/s with 10/10 quality, but requires ~15 GB KV cache at 128K context — plan memory accordingly.
...
| qwen3-coder:30b | 55.7 tok/s | 10/10 | 18 GB | 15.2 GB | 7.8s |
...
The benchmark testbed runs on a MacBook Pro M1 Max (64 GB unified memory) with Ollama serving models via `/api/generate`. All measurements use consistent methodology — see the [Methodology](methodology.md) page for details.
